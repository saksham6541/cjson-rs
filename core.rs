...
// --- Compatibility / divergence coverage (Task 3) ---
// These two cases were confirmed by the differential fuzz_driver (Task 2 /
// DECISIONS.md [Task 2]). They are currently deliberate *stricter* Rust
// behaviour relative to upstream cJSON. Tests lock that behaviour in so a
// later "match cJSON" change is visible as a failing test rather than a
// silent semantic flip. See DECISIONS.md for the open fix-vs-document decision.

#[test]
fn rejects_raw_control_characters_inside_string_literals() {
    // cJSON accepts raw C0 controls (0x00-0x1F) inside strings and re-escapes
    // them on print (e.g. \u0018). Rust rejects them per JSON / RFC 8259.
    // Minimised reproducer from fuzz: {"a":"x\x18y"}
    let input = "{\"a\":\"x\u{0018}y\"}";
    let err = parse(input).expect_err("raw control byte in string must be rejected");
    assert!(
        err.to_string().contains("control characters are not allowed in strings"),
        "unexpected error text: {err}"
    );

    // Neighbouring controls should also be rejected.
    for code in [0x01u8, 0x0A, 0x1F] {
        let s = format!("{{\"k\":\"x{}y\"}}", code as char);
        assert!(
            parse(&s).is_err(),
            "expected reject for control 0x{code:02x}"
        );
    }
...
fn rejects_trailing_garbage_after_a_complete_value() {
    ...
    // content. Rust requires the entire input to be consumed.
    // Minimised reproducer from fuzz: nullXXXXX
    let err = parse("nullXXXXX").expect_err("trailing garbage must be rejected");
    assert!(
        err.to_string().contains("unexpected trailing input"),
        "unexpected error text: {err}"
    );

    assert!(parse("true   ").is_ok()); // trailing whitespace is fine
    assert!(parse("true#comment").is_err());
